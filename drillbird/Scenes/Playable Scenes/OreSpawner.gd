...
func SpawnOreAtLocation(location:Vector2,ore:abstract_ore,velocity:Vector2,cooldown:bool=true,placedByGhost:bool=false,customCooldown:float=-1):
	...
	node.SetOreType(ore,cooldown,placedByGhost,customCooldown)
	if ore.ID==10:
